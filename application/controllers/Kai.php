...
class Kai extends CI_Controller {
	...
	function __construct()
	{
		parent::__construct();
		$this->load->database();
		$this->load->model('M_kai');
	}
	
	public function index(){
		$gmapsallKAI = $this->M_kai->gmapsallKAI();
		// $markerKAI = array();
		...
		$getMarkerMapsDashboardsToday = $this->M_kai->getMarkerMapsDashboardsToday();
		$markerToday = array();
		...
		$conditionVMKAI = $this->M_kai->conditionVMKAI();
		...
		$getTotalUserKAI = $this->M_kai->getTotalUserKAI();
		$data['data_userK'] = json_encode($getTotalUserKAI);
		$getDatadayK = $this->M_kai->getDatadayK();
		$data['data_dayK'] = json_encode($getDatadayK);
		$getDatamonthK = $this->M_kai->getDatamonthK();
		$data['data_monthK'] = json_encode($getDatamonthK);
		$getDatayearK = $this->M_kai->getDatayearK();
		$data['data_yearK'] = json_encode($getDatayearK);

		$trafficallKAI = $this->M_kai->trafficallKAI();
		...
		$getTop10KAI = $this->M_kai->getTop10KAI();
		...
		$data['title']='Vending Machine - KAI';
		$this->load->view('main/kai',$data);
	}
