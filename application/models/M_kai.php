...
class M_kai extends CI_Model{
	...
	function getMarkerMapsDashboardsToday(){
		...
		FROM mesin
		JOIN transaksi ON mesin.`id_mesin` = transaksi.`id_mesin`
		WHERE DAY(waktu_transaksi) = DAY(CURDATE()) AND kategori = 'KAI'
		GROUP BY id_mesin
		";
		return $query = $this->db->query($sql)->result();
	}
	...
	function getDatamonthK(){
		...
		FROM transaksi
		JOIN mesin ON mesin.`id_mesin`=transaksi.`id_mesin`
		WHERE MONTH(waktu_transaksi) = MONTH(CURDATE())  AND kategori = 'KAI'
		";
		return $query = $this->db->query($sql)->result();
	}
	function getDatayearK(){
		...
		FROM transaksi
		JOIN mesin ON mesin.`id_mesin`=transaksi.`id_mesin`
		WHERE YEAR(waktu_transaksi) = YEAR(CURDATE())  AND kategori = 'KAI'
		";
		return $query = $this->db->query($sql)->result();
	}
